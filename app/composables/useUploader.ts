...
const api = 'http://parentapi.test/api'
...
function createFileSchema() {
  ...
  return z.object({
    file: z
      .custom<File>((v) => FileCtor && v instanceof FileCtor, 'Please choose a file')
      .refine((f) => !!f, 'Please choose a file')
      .refine((f) => ACCEPTED_TYPES.includes(f!.type), 'Only PDF, PNG or JPG are allowed')
...
export function useUploader() {
    ...
    try {
      ...
      const res = await axios.post(`{api}/upload`, fd, {
        onUploadProgress: (evt) => {
          if (evt.total) progress.value = Math.round((evt.loaded * 100) / evt.total)
        }
      })
      ...
      status.value = 'failed'
      uploading.value = false
      t.add({ title: 'Upload failed', description: error.value, color: 'red', icon: 'i-heroicons-x-circle' })
    }
  }
  ...
  const tick = async () => {
    ...
    try {
      const { data } = await axios.get(`${api}/upload/status/${docId.value}`)
      ...
      if (data.status === 'failed') {
        t.add({ title: 'Processing failed', color: 'red', icon: 'i-heroicons-exclamation-triangle' })
        stopPolling()
        return
      }
    } catch {
